...
const METRICS: MetricDef[] = [
  { ringIdx: 0, phase: 0,             speed: 0.22, icon: "⚡", label: "Health Score",  value: "87",   unit: "/100",   color: "#10b981" },
  { ringIdx: 0, phase: Math.PI,       speed: 0.22, icon: "✓",  label: "Decisions",     value: "8",    unit: " made",  color: "#3d8bff" },
  { ringIdx: 1, phase: 0.80,          speed: 0.18, icon: "→",  label: "Action Items",  value: "12",   unit: " items", color: "#f5b94b" },
  { ringIdx: 1, phase: Math.PI + 0.8, speed: 0.18, icon: "⏱", label: "Time Wasted",   value: "18",   unit: " min",   color: "#f87171" },
  { ringIdx: 2, phase: 1.50,          speed: 0.28, icon: "◎",  label: "Talk Balance",  value: "Fair", unit: "",       color: "#22d3ee" },
];
...
/** 3-D position on an inclined circular orbit. */
...
/** Soft radial-glow canvas texture shared across sprites. */
...
/** Draw one complete inclined orbit circle as a THREE.Line. */
...
/**
 * DashboardGlobe — Option 2 hero visual.
 *
 * A glowing wireframe planet with five metric stat cards orbiting it on
 ...
 *
 * Built on Three.js (already a project dep). Purely decorative (aria-hidden).
 * Must be client-rendered (SSR disabled by the dynamic() caller in hero.tsx).
 */
export function DashboardGlobe({ className = "" }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    /* ── Equatorial scan group (spins at its own rate inside root) ── */
    const scanGroup = new THREE.Group();
    root.add(scanGroup);

    // Full dim equatorial ring
    ...
    /* ── Metric anchor dots (glowing points at each card's orbital position) ── */
    ...
    /* ── Connector lines: globe surface point → metric card position ── */
    // LineSegments reads vertices in pairs: (v0,v1), (v2,v3) … each pair = one segment.
    const connBuf = new Float32Array(METRICS.length * 6); // 2 vec3 per metric
    ...
    const cardEls = METRICS.map(m => {
      ...
      valEl.style.cssText = `font-family:var(--font-display,system-ui,sans-serif);font-size:${isLong ? "15" : "22"}px;font-weight:800;line-height:1.1;color:${m.color};letter-spacing:-0.03em;text-shadow:0 0 16px ${m.color}80`;
    ...
    const tick = () => {
      ...
      mPos.forEach((p, i) => {
        const surf = p.clone().normalize().multiplyScalar(1.54); // globe surface
      ...
      /* Project metric 3-D positions to 2-D and update HTML card DOM */
      ...
      mPos.forEach((p, i) => {
        ...
        // Depth: 0 = far side of orbit, 1 = near side
